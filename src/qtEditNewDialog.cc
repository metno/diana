...
{
  //  DEBUG_ << "EditNewDialog::Selected tab:" <<tabindex;
  normal= (tabindex == 0);
...
void EditNewDialog::combineSelect(QListWidgetItem * item)
{
  ...
  //  DEBUG_ << "EditNewDialog::Combineselect:" << s;
...
void EditNewDialog::prodtimechanged(int v)
{
  ...
  //  DEBUG_ << "EditNewDialog::Prodtime changed:" << prodtime;
...
{
  //DEBUG_ << "EditNewDialog::prodBox called";
  int n= products.size();
  ...
  currprod= idx;
  //DEBUG_ << "....Selected Product:" << products[idx].name;
...
void EditNewDialog::idBox(int idx)
{
  ...
  //DEBUG_ << "....Selected Pid:" << products[currprod].pids[idx].name;
...
bool EditNewDialog::load(editDBinfo& edbi){
  ...
  if( m_ctrl && m_editm ){
    ...
    //DEBUG_ << "EditNewDialog::load - No controller!";
    checkStatus();
    return false;
  }
...
miutil::miString EditNewDialog::savedProd2Str(const savedProduct& sp,
    const miutil::miString undef)
{
  if (sp.ptime.undef())
    return undef;
  else
    return sp.pid + miutil::miString(" - ") + sp.ptime.isoTime();
}


bool EditNewDialog::setNormal()
{
  //DEBUG_ << "EditNewDialog::setNormal called";
  isdata= false;
  ...
  for (int i=0; i<n && i<maxelements-1; i++){
    ...
    if (products[currprod].fields[i].fromfield){
      if (products[currprod].fields[i].fromfname.exists())
        ...
        s= miutil::miString("<font color=\"blue\"> ") + tr("Field undefined").toStdString() +
        miutil::miString(" </font> ");
    }
    else
      s= miutil::miString("<font color=\"red\"> ") +
...
void EditNewDialog::handleObjectButton(int num)
{
  if( m_editm ){
    ...
    WARN_ << "EditNewDialog::handleelementButton - No controller!";
  }
  setObjectLabel();
  checkStatus();
}
...
void EditNewDialog::handleFieldButton(int num)
{
  if( m_editm ){
    ...
    //DEBUG_ << "EditNewDialog::handleelementButton - No controller!";
  }
  setFieldLabel();
  checkStatus();
}
...
bool EditNewDialog::load_combine(){
  //DEBUG_ << "EditNewDialog::Load-combine";
  isdata= false;
  if (pid.sendable && !dbi.loggedin) {
    checkStatus();
    return false;
  ...
  if( m_editm ){
    ...
    if (n>0) {
      ...
      WARN_ << "EditNewDialog::load - no analyses found";
      checkStatus();
      combineClear();
      return false;
    }
...
void EditNewDialog::login_clicked()
{
  ...
  while (1) {
    ...
    if (!dbi.user.exists() || !dbi.host.exists()){
      ...
      //       DEBUG_ << "Trying host:" << dbi.host << " User:" << dbi.user << " Pass:"
      // 	   << dbi.pass;
      dbi.loggedin= m_editm->loginDatabase(dbi,message);
    }
    if (dbi.loggedin) {
      //       DEBUG_ << "Success...logged in";
      emit newLogin(dbi);
      ...
      message= miutil::miString(tr("Can not log in. Message from server:\n").toStdString()) +message;
...
void EditNewDialog::ok_clicked(){
  ...
  //   if (msg.exists()) {
  //     miutil::miString pname= prodbox->currentText().toStdString();
  //     miutil::miString message= pname + "\n" + msg;
  ...
  if (!productfree){
    // give error
    //miutil::miString message= "Produktet er ikke tilgjengelig.\n Kan ikke starte produksjon";

...
void ObsTemp::init(ObsPlot *oplot, vector<int> &levels){

  //It is assumed that each station has only one entry in the temp
  //file.
...
int ObsTemp::getIndex(int& level,
		      float& pmin,
		      float& pmax,
		      vector<float> & PPPP)
{
...
void
ObsTemp::putData(int i, int levelIndex,miTime obsTime,ObsData &d)
{

...
int            VcrossPlot::zoomSpec[4]= { 0,0,0,0 };
...
VcrossPlot::VcrossPlot()
    : vcopt(0), numLev(0), nPoint(0), nTotal(0), timeGraph(false),
      horizontalPosNum(0)
{
#ifdef DEBUGPRINT
...
bool VcrossPlot::parseSetup(SetupParser& sp)
{
  ...
  for (int f=0; f<numVcrossFunctions; f++) {
    ...
    for (size_t k=k1+1; k<k2; k++)
      if (vcrossFunctionDefs[f][k]==',') narg++;
  ...
  if (sp.getSection(section2,vstr)) {
	...
	if (parts.size()==2) {
	  if (CommandParser::isInt(parts[1])) {
	    ...
	    if (vcParName.find(name)==vcParName.end() &&
		vcParNumber.find(number)==vcParNumber.end()) {
  ...
  if (sp.getSection(section3,vstr)) {
	...
	if (parts.size()==2) {
	  ...
	  if (k1!=string::npos && k2!=string::npos && k2>k1+1) {
	    ...
	    names= str.split(',');
	    int j= 0;
	    while (j<numVcrossFunctions &&
		   func!=vcrossFunctionNames[j]) j++;
	    if (j<numVcrossFunctions &&
		names.size()==vcrossFunctionArguments[j]) {
	...
	if (parts.size()==2) {
	  if (parts[0]=="name") {
	    ...
	    if (k1!=string::npos && k2!=string::npos && k2>k1+1) {
	      ...
	      names= str.split(',');
	      int j= 0;
	      while (j<numVcrossPlotTypes &&
		     func!=vcrossPlotNames[j]) j++;
	      if (j<numVcrossPlotTypes &&
		  names.size()==vcrossPlotArguments[j]) {
	        for (int i=0; i<names.size(); i++) {
		  if (definedNames.find(names[i])==definedNames.end()) {
	            msg= "Input name not defined: " + names[i];
		  }
  ...
  for (int i=0; i<np; i++) {
    ...
    if (pn==pnend) {
      ...
      if (iparam[i]!=1 && iparam[i]!=8)
        cerr<<"VCROSS: parameter not defined in setup: "
	    <<iparam[i]<<endl;
    } else {
  ...
  while (paramdef.size()>numfound) {
    ...
    for (vf=vfbegin; vf!=vfend; vf++) {
      if (paramdef.find(vf->first)==paramdef.end()) {
        ...
        int i= 0;
        while (i<n &&
	       paramdef.find(vf->second.vars[i])!=paramdef.end()) i++;
        if (i==n) {
//##############################################################
...
//##############################################################
  ...
  for (int j=0; j<m; j++) {
    ...
    if (f!=fend) {
      ...
      while (i<n && paramdef.find(f->second.vars[i])!=pend) i++;
...
map<miString,miString> VcrossPlot::getAllFieldOptions(SetupParser& sp)
{
  ...
  while (vcf!=vcfend) {
    ...
    if (po.linetype.name.length()==0) po.linetype= defaultLinetype;
    if (po.undefLinetype.name.length()==0) po.undefLinetype= defaultLinetype;
	...
	for (int j=1; j<po.colours.size(); j++)
	...
	for (int j=1; j<po.linetypes.size(); j++)
	...
	for (int j=1; j<po.linewidths.size(); j++)
	...
	if (po.zeroLine>=0) ostr << " zero.line=" << po.zeroLine;
      }
//    ostr << " extreme.type="   << po.extremeType
//	   << " extreme.size="   << po.extremeSize
//	   << " extreme.radius=" << po.extremeRadius
      ostr << " line.smooth="    << po.lineSmooth
	   << " value.label="    << po.valueLabel
	   << " label.size="     << po.labelSize;
      ostr << " base=" << po.base;
...
//	   << " vector.unit="<< po.vectorunit
//	   << " rel.size="   << po.relsize;
    } else if (vcf->second.plotType==vcpt_vt_omega ||
	       vcf->second.plotType==vcpt_vt_w) {
      ostr << "colour="      << po.linecolour.Name()
	   << " linewidth="  << po.linewidth
	   << " density="    << po.density
	   << " vector.unit="<< po.vectorunit
...
bool VcrossPlot::plot(VcrossOptions *vcoptions,
  ...
  if (vcoord==5) {
    ...
    if (vs[1]=="fl" || vs[1]=="ft") vcaxis="Ft";  // FL correct name here ???
    else             vcaxis="m";
  } else if (vs[0]=="pressure" && npp>=0) {
    vcplot= vcv_pressure;
    if (vs[1]=="fl") vcaxis="FL";
    else             vcaxis="hPa";
  ...
  if (vcplot==vcv_none) return false;
...
int VcrossPlot::addPar1d(int param, float *pdata) {
  ...
  if (pdata==0) pdata= new float[nPoint];
...
int VcrossPlot::addPar2d(int param, float *pdata) {
  ...
  if (pdata==0) pdata= new float[nTotal];
...
void VcrossPlot::addHCStencil(const int& size,
...
void VcrossPlot::addHCScissor(const int x0, const int y0,
...
bool VcrossPlot::prepareData(const miString& fileName) {
  ...
  if (vcoord!=2  && vcoord!=10 && vcoord!=1 &&
      vcoord!=4  && vcoord!=5  && vcoord!=11 && vcoord!=12) {
    cerr << "VcrossPlot::prepareData : Unknown vertical coordinate "
	 << vcoord << endl;
    return false;
  }
...
//##################################################################################
  ...
  for (n=0; n<numPar1d; n++) {
    // ps (vcoord=2,10,4,12)
    if (idPar1d[n]==8)     nps=n;
    // topography
    if (idPar1d[n]==101)   ntopo=n;
    // sea surface elevation (vcoord=5)
    if (idPar1d[n]==301)   npss=n;
    // sea bottom (vcoord=5)
    if (idPar1d[n]==351)   npsb=n;

    // the following are parameters computed in program vcdata

    // x_grid
    if (idPar1d[n]==-1001) nxg=n;
    // y_grid
    if (idPar1d[n]==-1002) nyg=n;
    // s_grid (in unit m) ... low scaling (ds_grid better...)
    if (idPar1d[n]==-1003) nxs=n;
    // coriolis parameter
    if (idPar1d[n]==-1004) ncor=n;
    // latitude
    if (idPar1d[n]==-1005) nlat=n;
    // longitude
    if (idPar1d[n]==-1006) nlon=n;
    // ds_grid (in unit m) .. length increments
    if (idPar1d[n]==-1007) nxds=n;
    // rotation of vector components to E/W and N/S
    if (idPar1d[n]==-1008) ngdir1=n;
    if (idPar1d[n]==-1009) ngdir2=n;
  }
  ...
  for (n=0; n<numPar2d; n++) {
    ...
    if (idPar2d[n]==8) npp=n;
    if (idPar2d[n]==1) nzz=n;
  }
  ...
  if (error) {
    cerr<<"VcrossPlot::prepareData : Not able to plot this crossection"<<endl;
    return false;
  }
  ...
  if (vcoord==2 || vcoord==10) {
    ...
    if (npsb>=0) npy1= addPar1d(-30003);
    if (npss>=0) npy2= addPar1d(-30005);
...
void VcrossPlot::prepareVertical() {
  ...
  if (vcoord!=5 && vcoord!=11) {
    ...
    if (vcoordPlot==vcv_exner) {
      ...
      n= 0;
      for (n=0; n<nTotal; n++) {
        if (zmin>cdata2d[nzz][n]) zmin=cdata2d[nzz][n];
        if (zmax<cdata2d[nzz][n]) zmax=cdata2d[nzz][n];
      }
      zmin-=40.;
      zmax+=40.;
    ...
    if (nps>=0 && vcoordPlot==vcv_exner) {
      ...
      for (i=0; i<nPoint; i++) {
        k= 0;
        while (k<numLev && cdata2d[ny][k*nPoint+i]==fieldUndef) k++;
        if (k<numLev) {
          float dy= 0.;
          if (k>0 && k<numLev-1 &&
	      cdata2d[ny][(k+1)*nPoint+i]!=fieldUndef)
            dy= cdata2d[ny][(k+1)*nPoint+i] - cdata2d[ny][k*nPoint+i];
...
//####################################################################
  ...
  if (timeGraph) {
	...
	xDatamax= cdata1d[nxs][nPoint-1];
      ...
      if (minutes<0) cerr<<"!!!!!!!!! minutes= "<<minutes<<" !!!!!!!"<<endl;
      if (minutes<0) minutes= -minutes;

      timeGraphMinuteStep= (xDatamax-xDatamin)/float(minutes);
    } else {
      for (i=0; i<nPoint; i++) {
        int minutes= miTime::minDiff(validTimeSeries[i],
			             timeGraphReference);
			 ...
			 vector<int> parloc)
{
  ...
  switch (vcfunc) {
    ...
    case vcf_add:      if (compute==0) compute= 1;
    case vcf_subtract: if (compute==0) compute= 2;
    case vcf_multiply: if (compute==0) compute= 3;
    ...
    case vcf_tc_from_th:     if (compute==0) compute= 1;
    case vcf_tk_from_th:     if (compute==0) compute= 2;
    case vcf_th_from_tk:     if (compute==0) compute= 3;
    case vcf_thesat_from_tk: if (compute==0) compute= 4;
    ...
    case vcf_the_from_tk_q: if (compute==0) compute= 1;
    case vcf_the_from_th_q: if (compute==0) compute= 2;
      n1= parloc[0];
      n2= parloc[1];
      if (npp<0) return -1;
    ...
    case vcf_rh_from_tk_q:  if (compute==0) compute= 1;
    case vcf_rh_from_th_q:  if (compute==0) compute= 2;
    case vcf_q_from_tk_rh:  if (compute==0) compute= 3;
    case vcf_q_from_th_rh:  if (compute==0) compute= 4;
    case vcf_tdc_from_tk_q:  if (compute==0) compute= 5;
    case vcf_tdc_from_th_q:  if (compute==0) compute= 6;
    case vcf_tdc_from_tk_rh: if (compute==0) compute= 7;
    case vcf_tdc_from_th_rh: if (compute==0) compute= 8;
    case vcf_tdk_from_tk_q:  if (compute==0) compute= 9;
    case vcf_tdk_from_th_q:  if (compute==0) compute= 10;
    case vcf_tdk_from_tk_rh: if (compute==0) compute= 11;
    case vcf_tdk_from_th_rh: if (compute==0) compute= 12;
      n1= parloc[0];
      n2= parloc[1];
      if (npp<0) return -1;
    ...
    case vcf_ducting_from_tk_q:       if (compute==0) compute= 1;
    case vcf_ducting_from_th_q:       if (compute==0) compute= 2;
    case vcf_ducting_from_tk_rh:      if (compute==0) compute= 3;
    case vcf_ducting_from_th_rh:      if (compute==0) compute= 4;
    case vcf_d_ducting_dz_from_tk_q:  if (compute==0) compute= 5;
    case vcf_d_ducting_dz_from_th_q:  if (compute==0) compute= 6;
    case vcf_d_ducting_dz_from_tk_rh: if (compute==0) compute= 7;
    case vcf_d_ducting_dz_from_th_rh: if (compute==0) compute= 8;
      n1= parloc[0];
      n2= parloc[1];
      if (npp<0) return -1;
      compddz= compute;
	...
	return -1;
	  ...
	  ntk= n1;
	      ...
	      if (cdata2d[ntk][n]!=fieldUndef && cdata2d[ntk][n+nPoint]!=fieldUndef &&
	          cdata2d[no][n] !=fieldUndef && cdata2d[no][n+nPoint] !=fieldUndef) {
	  ...
	  nth= n1;
          ...
          for (k=0; k<numLev-1; k++) {
            for (i=0, n=nPoint*k; i<nPoint; i++, n++) {
	      if (cdata2d[nth][n]!=fieldUndef && cdata2d[nth][n+nPoint]!=fieldUndef &&
	          cdata2d[no][n] !=fieldUndef && cdata2d[no][n+nPoint] !=fieldUndef) {
	        th1= cdata2d[nth][n];
    ...
    case vcf_ff_total:
      ...
      if (allDefined) {
        ...
        for (n=0; n<nTotal; n++) {
          if (cdata2d[nu][n]!=fieldUndef &&
	      cdata2d[nv][n]!=fieldUndef) {
            u= cdata2d[nu][n];
            v= cdata2d[nv][n];
            cdata2d[no][n]= sqrtf(u*u+v*v);
          } else {
    ...
    case vcf_ff_normal:      if (compute==0) compute= 1;
    case vcf_ff_tangential:  if (compute==0) compute= 2;
    case vcf_ff_north_south: if (compute==0) compute= 3;
    case vcf_ff_east_west:   if (compute==0) compute= 4;
      nu= parloc[0];
      nv= parloc[1];
      if (compute==1) {
	nrot1= nsin;
    ...
    case vcf_momentum_vn_fs: // m = FFnormal + f*s,  f=coriolis, s=length(incl. map.ratio)
      nvn= parloc[0];
      if (ncor<0 || nxs<0) return -1;
      if (allDefined) {
        for (k=0; k<numLev; k++) {
          for (i=0, n=nPoint*k; i<nPoint; i++, n++) {
            cdata2d[no][n]=  cdata2d[nvn][n]
                            +cdata1d[ncor][i]*cdata1d[nxs][i];
          }
        }
      } else {
        for (k=0; k<numLev; k++) {
    ...
    case vcf_height_above_msl_from_th:     if (compute==0) compute= 1;  // height above msl
    case vcf_height_above_surface_from_th: if (compute==0) compute= 2;  // height above surface
      nth= parloc[0];
      ...
      float alvl1,blvl1,alvl2,blvl2,p2,pi1,pi2,dz,z1,px,pim1,pip1,dthdpi,
            pifull,thhalf;
      int   km,kp;
      for (i=0; i<nPoint; i++)
	...
	kp= (k<numLev-1) ? k+1 : numLev-1;
	  ...
	  // so we make a first guess of a temperature profile to comp. height
          ...
          dthdpi= (cdata2d[nth][km*nPoint+i]-cdata2d[nth][kp*nPoint+i])/(pim1-pip1);
...
bool VcrossPlot::computeSize()
{
  ...
  if (xframe1<dchx) xframe1= dchx;
  if (xframe2<dchx) xframe2= dchx;
  if (yframe1<dchy) yframe1= dchy;
  if (yframe2<dchy) yframe2= dchy;
  ...
  if (!timeGraph) {
    ...
    if (vcopt->pDistance) {
      ...
      if (xframe1<chxt*4.6) xframe1= chxt*4.6;
      if (xframe2<chxt*1.6) xframe2= chxt*1.6;
    }
    // x,y coordinates
    if (vcopt->pXYpos) {
    ...
    if (vcopt->pText) {
      ...
      if (xframe1<chxt*2.5) xframe1= chxt*2.5;
      if (xframe2<chxt*2.5) xframe2= chxt*2.5;
    }
  ...
  if (dx>plotw*0.4 || dy>ploth*0.4) {
    if (dx>plotw*0.4) dx= plotw*0.4;
    if (dy>ploth*0.4) dy= ploth*0.4;
  ...
  if (zoomType!=vczoom_keep) {
	...
	yPlotmin= yDatamin + (yDatamax-yDatamin)*float(vcopt->minVerticalArea)*0.01;
	yPlotmax= yDatamin + (yDatamax-yDatamin)*float(vcopt->maxVerticalArea)*0.01;
	hfree= false;
      }
      if (vcopt->stdHorizontalArea) {
	xPlotmin= xDatamin + (xDatamax-xDatamin)*float(vcopt->minHorizontalArea)*0.01;
	xPlotmax= xDatamin + (xDatamax-xDatamin)*float(vcopt->maxHorizontalArea)*0.01;
	wfree= false;
      }
    } else {
  ...
  if (zoomType!=vczoom_standard) zoomType= vczoom_keep;
...
************************************************************************/
  ...
  int   i,k,k1,k2,l,n,ibgn,iend;
  float p,pi,x,x1,x2,y,y1,y2,y3,dx,dy,xlen,xcut;

  const int npfixed1= 17;
  float pfixed1[npfixed1]= { 1000.,925.,850.,700.,600.,500.,400.,300.,
			     250.,200.,150.,100.,70.,50.,30.,10.,5. };
  const int npfixed2= 22;
  float pfixed2[npfixed2]= { 1000.,950.,900.,850.,800.,750.,700.,650.,
			     600.,550.,500.,450.,400.,350.,300.,250.,
			     200.,150.,100., 50., 10.,  5. };
  const int nzsteps=10;
  float zsteps[nzsteps]= { 5.,10.,25.,50.,100.,250.,500.,1000.,2500.,5000. };

  const int nflsteps=8;
  float flsteps[nflsteps]= { 1.,2.,5.,10.,50.,100.,200.,500. };

  // P -> FlightLevels (used for remapping fields from P to FL)
  const int mfl= 16;
  const int plevels[mfl]= {1000, 925, 850, 700, 600, 500, 400, 300,
			    250, 200, 150, 100,  70,  50,  30,  10 };
  const int flevels[mfl]= {   0,  25,  50, 100, 140, 180, 240, 300,
  ...
  if (vcoord!=5 && vcoord!=11) {
    ...
    if (vcoordPlot==vcv_exner ) {
      ...
      if (verticalAxis=="m") {
        n=1;
        while (n<nzsteps && zsteps[n]*10.<dz) n++;
        n--;
	...
	dz/=fl2m;
        n=1;
        while (n<nflsteps && flsteps[n]*10.<dz) n++;
        n--;
  ...
  for (i=0; i<nPoint-1; i++) {
    if (cdata1d[nxs][i]<xPlotmin) ipd1=i;
    if (cdata1d[nxs][i]<xPlotmax) ipd2=i+1;
  }
  ...
  ilev2= 1;
  for (k=0; k<numLev-1; k++) {
    if (vlimitmax[k]<yPlotmin) ilev1= k;
...
// 	      }
	  ...
	  glShadeModel(GL_FLAT);
        ...
        izstp= 500;
        if (izmax-izmin>6000) izstp=1000;
...
************************************************************************/
  ...
  if (!timeGraph) {
    ...
    if (vcopt->pDistance) {
      ...
      i= int(refPosition);
      if (i<0)        i= 0;
      if (i>nPoint-2) i= nPoint-2;
      float rpos= cdata1d[nxs][i]
	         +(cdata1d[nxs][i+1]-cdata1d[nxs][i])*(refPosition-float(i));
      float unit;
      miString uname;
	...
	for (i=ip1; i<=ip2; i++) {
	  if ((cdata1d[nxs][i]>x+xlen && cdata1d[nxs][i]<xcut) || i==ip2) {
	    ostringstream xostr;
	    xostr<<setprecision(1)<<setiosflags(ios::fixed)
		 <<fabsf((cdata1d[nxs][i]-rpos)/unit);
    ...
    if (vcopt->pGeoPos) {
	  ...
	  ostringstream xostr,yostr;
	  xostr<<setw(5)<<setprecision(1)<<setiosflags(ios::fixed)<<fabsf(glat);
	  if (glat<0.) xostr << 'S';
	  else         xostr << 'N';
	  yostr<<setw(5)<<setprecision(1)<<setiosflags(ios::fixed)<<fabsf(glon);
	  if (glon<0.) yostr << 'W';
	  else         yostr << 'E';
	  string xstr= xostr.str();
	  string ystr= yostr.str();
	...
	string str= ostr.str();
	if (lmax<str.length()) lmax= str.length();
	fctext[i]= str;
      }
	...
	if (dxhr>dx) dxhr= dx;
      ...
      chxt= chx*0.75;
      if (chxt*3.>dxhr)  chxt= dxhr/3.;
      if (chxt*6.>dxday) chxt= dxday/6.;
      if (chxt*(lmax+1)>dxday) chxt =dxday/(lmax+1);
      if (ittmin==ip1)
        showday[ip1]= true;
...
//----------------------------------------------------------
  ...
  if (vcopt->pPositionNames && markName.size()>0 && !timeGraph && nxs>=0) {
    ...
    for (n=0; n<nn; n++) {
      ...
      if (i<0)        i= 0;
      if (i>nPoint-2) i= nPoint-2;
      x1= cdata1d[nxs][i] + (cdata1d[nxs][i+1]-cdata1d[nxs][i])*(x-float(i));
      x= markNamePosMax[n];
      i= int(x);
      if (i<0)        i= 0;
      if (i>nPoint-2) i= nPoint-2;
      x2= cdata1d[nxs][i] + (cdata1d[nxs][i+1]-cdata1d[nxs][i])*(x-float(i));
      if (x2>xDatamin && x1<xDatamax) {
	fp->getStringSize(markName[n].cStr(),dx,dy);
	dxh=dx*0.5+chxt;
	xm= (x1+x2)*0.5;
	if (x1<xWindowmin+dxh) x1= xWindowmin+dxh;
	if (x2>xWindowmax-dxh) x2= xWindowmax-dxh;
	if (x1<=x2) {
	  if (xm<x1) xm= x1;
	  if (xm>x2) xm= x2;
	  vpos.push_back(n);
...
//	  x1= x2-dxh*2.;
	...
	if (x1<vx1[n]-dxh) x1= vx1[n]-dxh;
	if (x2>vx2[n]+dxh) x2= vx2[n]+dxh;
	if (x2-x1>dxh*1.99) {
	  x=(x1+x2)*0.5;
	  vxm[n]= x;
	  x-=vdx[n]*0.5;
  ...
  for (int loop=0; loop<3; loop++) {
    ...
    if (k1>=0) {
      if (k1<ilev1) k1= ilev1;
      if (k2>ilev2) k2= ilev2;
      k2++;
    }
    if (k1<k2) {
      if (c==backColour) c= contrastColour;
      ...
      if (vcoord!=4) {
        for (k=k1; k<k2; k++)
	  xyclip(npd,&cdata2d[nx][k*nPoint+ipd1],
		     &cdata2d[ny][k*nPoint+ipd1],xylim);
      } else {
	// theta levels
        for (k=k1; k<k2; k++) {
          i= ipd1-1;
          while (i<ipd2) {
            i++;
            while (i<=ipd2 && cdata2d[ny][k*nPoint+i]==fieldUndef) i++;
            ibgn= i;
            while (i<=ipd2 && cdata2d[ny][k*nPoint+i]!=fieldUndef) i++;
	    iend= i;
            if (ibgn<iend-1)
  ...
  if (vcopt->pSurface && npy1>=0) {
    ...
    if (vcoord!=5) {
      ...
      while (i<ipd2) {
        i++;
        while (i<=ipd2 && cdata1d[npy1][i]==fieldUndef) i++;
        ibgn= i;
        while (i<=ipd2 && cdata1d[npy1][i]!=fieldUndef) i++;
	iend= i-1;
        if (ibgn<=ipd2) {
	  ...
	  xyclip(iend-ibgn+1,&cdata1d[nxs][ibgn],
		             &cdata1d[npy1][ibgn],xylim);
	  if (iend<ipd2) {
  ...
  if (vcopt->pMarkerlines) {
    ...
    if (vcoordPlot==vcv_height) {
      for (n=0; n<numPar1d; n++) {
        if (idPar1d[n]==907) mlines.push_back(n); // inflight height above sealevel
      }
    } else if (vcoordPlot==vcv_exner || vcoordPlot==vcv_pressure) {
      for (n=0; n<numPar1d; n++) {
        if (idPar1d[n]==908) mlines.push_back(n); // inflight pressure
      }
    }
    if (mlines.size()>0) {
      ...
      for (int m=0; m<mlines.size(); m++) {
        ...
        while (i<ipd2) {
          i++;
          while (i<=ipd2 && cdata1d[n][i]==fieldUndef) i++;
          ibgn= i;
          while (i<=ipd2 && cdata1d[n][i]!=fieldUndef) i++;
	  iend= i;
          if (ibgn<iend-1) {
  ...
  for(int i=0;i<nlabels;i++){
	...
	for(int k=0; k<nstokens; k++){
	  ...
	  for(int l=0; l<nsstokens; l++){
	    vector<miString> ssstokens = sstokens[l].split('\"','\"',"=",true);
	    if(ssstokens.size()==2){
  ...
  if (pfixed)  delete[] pfixed;
  if (ypfixed) delete[] ypfixed;

  return true;
}


bool VcrossPlot::plotData(const miString& fieldname,
		          PlotOptions& poptions)
{
#ifdef DEBUGPRINT
  ...
  for (i=0; i<nPoint-1; i++) {
    if (cdata1d[nxs][i]<xPlotmin) ipd1=i;
    if (cdata1d[nxs][i]<xPlotmax) ipd2=i+1;
    if (cdata1d[nxs][i]<xPlotmin) iwpd1=i+1;
    if (cdata1d[nxs][i]<xPlotmax) iwpd2=i;
  }
  if (cdata1d[nxs][nPoint-1]<=xPlotmax) iwpd2=nPoint-1;
  ...
  for (k=0; k<numLev-1; k++) {
    if (vlimitmax[k]<yPlotmin) ilev1= k;
    if (vlimitmin[k]<yPlotmax) ilev2= k+1;
    if (vlimitmax[k]<yPlotmin) iwlev1= k+1;
    if (vlimitmin[k]<yPlotmax) iwlev2= k;
  }
  if (vlimitmin[numLev-1]<=yPlotmax) iwlev2= numLev-1;
  ...
  int iwpd[5]= { iwpd1, (iwpd1+iwpdm)/2, iwpdm, (iwpdm+iwpd2)/2, iwpd2 };
  int kprev=iwlev1;
  for (k=iwlev1+1; k<=iwlev2; k++) {
    float dy= 0.;
    for (int i=0; i<5; i++)
      dy+=(cdata2d[ny][k*nPoint+iwpd[i]]-cdata2d[ny][kprev*nPoint+iwpd[i]]);
    dy/=5.;
    if (dy>rwindAuto*0.8)
      kprev= k;
    else
  ...
  int   part[4]= { ipd1, ipd2, ilev1, ilev2 };
  int   partwind[4]= { iwpd1, iwpd2, iwlev1, iwlev2 };
  xylim[0]= xPlotmin;
  xylim[1]= xPlotmax;
  xylim[2]= yPlotmin;
  xylim[3]= yPlotmax;
  ...
  if (vcf->second.plotType==vcpt_contour) {
    ...
    if (bottomext) {
      if (nwork<0) nwork= addPar2d(-8888);
      for (i=0; i<nTotal; i++)
    ...
    if (bottomext && labfmt[0]!=0) {
      ...
      rbmap[3]= dx;
      // clear bitmap
      for (i=0; i<lbmap; i++) bmap[i]= 0;
      // mark area below bottom in the bitmap
      for (i=ipd1; i<ipd2; i++) {
        if (cdata1d[npy1][i]!=fieldUndef && cdata1d[npy1][i+1]!=fieldUndef) {
          ...
          if (yline[0]==fieldUndef) yline[0]=0.;
          if (yline[2]==fieldUndef) yline[2]=0.;
...
//          if (i>ipd1 || n>1) i1++;
	  if (n>0) i1++;
          if (i1<0) i1= 0;
          if (i2>=nxbmap) i2= nxbmap-1;
          for (int ixb=i1; ixb<=i2; ixb++) {
            x= rbmap[0]+rbmap[2]*(ixb-0.5);
            y= yline[n]+(yline[n+1]-yline[n])
                        *(x-xline[n])/(xline[n+1]-xline[n]);
            int iyb= int((y-rbmap[1])/rbmap[3])+1;
	    if (iyb>=nybmap) iyb= nybmap-1;
            for (j=0; j<iyb; j++) {
...
/*************************************************************************/
    ...
    if (!res) cerr<<"VcrossPlot::plotData  Contour error"<<endl;
    ...
    if (hardcopy && scissor) removeHCClipping();

    if (bottomext) {
      glDisable(GL_STENCIL_TEST);
      if (hardcopy && stencil) removeHCClipping();
    }
    ...
    plotWind(cdata2d[no1], cdata2d[no2], cdata2d[nx], cdata2d[ny],
	     partwind, ylim, rwindAuto, hstepAuto, windlevel, poptions);

  } else if (vcf->second.plotType==vcpt_vt_omega &&
  	     (vcoordPlot==vcv_exner || vcoordPlot==vcv_pressure)) {

    float hours= poptions.vectorunit;

    vcMovement(cdata2d[no1], cdata2d[no2], cdata2d[npp],
	       cdata2d[nx], cdata2d[ny],
	       partwind, ylim, hours, hstepAuto, windlevel, poptions);
    ...
    plotVector(cdata2d[no1], cdata2d[no2], cdata2d[nx], cdata2d[ny],
	       partwind, ylim, rwindAuto, hstepAuto, windlevel, poptions);
...
bool VcrossPlot::plotWind(float *u, float *v, float *x, float *y,
			  int *part, float *ylim,
  ...
  for (int iy=iy1; iy<iy2; iy++) {
   if (uselevel[iy]) {
    for (int ix=ix1; ix<ix2; ix+=hstep) {
      ...
      gx= x[i]; gy= y[i];
      if (u[i]!=fieldUndef &&
          v[i]!=fieldUndef && y[i]>=ymin && y[i]<=ymax) {
...
bool VcrossPlot::vector2fixedLevel(float *u, float *v, float *x, float *y,
  ...
  for (int ix=ix1; ix<=ix2; ix+=hstep) {
    ...
    for (int kf=kf1; kf<=kf2; kf++) {
      ...
      while (k<numLev && y[k*nPoint+ix]<=yfixed[kf]) k++;
	...
	// interpolation of direction and speed
        ...
        float u2= u[n2];
        float v2= v[n2];
        float ff= c1*sqrtf(u1*u1+v1*v1) + c2*sqrtf(u2*u2+v2*v2);
...
bool VcrossPlot::vcMovement(float *vt, float *wom, float *p, float *x, float *y,
  ...
  for (int iy=iy1; iy<iy2; iy++) {
	...
	if (y0>=ymin && y0<=ymax &&
	    vt[n]!=fieldUndef && wom[n]!=fieldUndef &&
	    (vt[n]!=0. || wom[n]!=0.)) {
...
bool VcrossPlot::plotVector(float *u, float *v, float *x, float *y,
			    int *part, float *ylim,
                            float size, int hstepAuto, bool *uselevel,
			    PlotOptions& poptions) {
#ifdef DEBUGPRINT
...
//############### bottomExtrapolate ...
void VcrossPlot::replaceUndefinedValues(int nx, int ny, float *f,
					bool fillAll)
{
...
//	  sum2+= 0.5*(f[ij+nx+1]-f[ij+2*nx+2]);
      ...
      if (num2>1) sum+=sum2;
      if (num>0) ftmp[ij]= sum/float(num);
    }
  }
...
vector<miString> VcrossPlot::split(const miString eString

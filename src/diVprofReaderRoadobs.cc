...
VprofValues_cpv VprofDataRoadobs::getValues(const std::string& name, const miutil::miTime& time, int /*realization*/)
{
  ...
  try {
    ...
    // read stationlist and init the api.
    // does nothing if already done
    ObsRoad road = ObsRoad(filename, db_connectfile, getStationsFileName(), db_parameterfile, time, NULL, false);
    VprofValues_p vv = road.getVprofPlot(getModelName(), name, time);
    vv->calculate();
    values.push_back(vv);
  } catch (...) {
    METLIBS_LOG_ERROR("Exception in ObsRoad: " << db_connectfile << "," << getStationsFileName() << "," << db_parameterfile << "," << time);
  }
#endif
  return values;
}

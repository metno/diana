...
FilledMap::FilledMap()
  : filename(""),timestamp(0),scale(1.0),tscale(1.0),
    opened(false),contexist(false),
    numPolytiles(0),polydata(0),
    numGroups(0),groups(0)
{
}

FilledMap::FilledMap(const miString fn)
  : filename(fn),timestamp(0),scale(1.0),tscale(1.0),
    opened(false),contexist(false),
    numPolytiles(0),polydata(0),
    numGroups(0),groups(0)
{
...
void FilledMap::clearGroups(){
  for (int i=0; i<numGroups; i++){
    if (groups[i].numtiles>0){
      if (groups[i].tilex)   delete[] groups[i].tilex;
      if (groups[i].tiley)   delete[] groups[i].tiley;
      if (groups[i].midlat)  delete[] groups[i].midlat;
      if (groups[i].midlon)  delete[] groups[i].midlon;
      if (groups[i].mmx)     delete[] groups[i].mmx;
      if (groups[i].mmy)     delete[] groups[i].mmy;
      if (groups[i].crecnr)  delete[] groups[i].crecnr;
      if (groups[i].cwp)     delete[] groups[i].cwp;
      if (groups[i].use)     delete[] groups[i].use;
      if (groups[i].tiletype)delete[] groups[i].tiletype;
      groups[i].numtiles= 0;
    }
  }
  numGroups= 0;
...
void FilledMap::clearPolys(){
  for (int i=0; i<numPolytiles; i++){
    if (polydata[i].polyverx) delete[] polydata[i].polyverx;
    if (polydata[i].polyvery) delete[] polydata[i].polyvery;
...
bool FilledMap::plot(Area area,               // current area
  ...
  if (opened){
    ...
    if (filechanged){
      cerr << "MAPFile changed on disk:" << filename
	   << " Current timestamp:" << ctimestamp
	   << " Old timestamp:" << timestamp << endl;
    }
  }
...
//       gspec[5] << endl;
	...
	if (groups[i].mmx[cidx+0] < minx) minx=groups[i].mmx[cidx+0];
	if (groups[i].mmx[cidx+1] > maxx) maxx=groups[i].mmx[cidx+1];
	if (groups[i].mmy[cidx+0] < miny) miny=groups[i].mmy[cidx+0];
	if (groups[i].mmy[cidx+1] > maxy) maxy=groups[i].mmy[cidx+1];
      }
      delete[] ctx;
      delete[] cty;
  ...
  for (int g=0; g<numGroups; g++){
    ...
    if ((groups[g].mmx[ngt*2+0] > x2) ||
	(groups[g].mmx[ngt*2+1] < x1) ||
	(groups[g].mmy[ngt*2+0] > y2) ||
	(groups[g].mmy[ngt*2+1] < y1)){
      continue;
    }
    // start tile loop
    for (int i=0; i<ngt; i++){
      // check if legal tile for this projection
      if (!groups[g].use[i]) continue;

      // check if tile inside area
      if ((groups[g].mmx[i*2+0] > x2) ||
	  (groups[g].mmx[i*2+1] < x1) ||
	  (groups[g].mmy[i*2+0] > y2) ||
	  (groups[g].mmy[i*2+1] < y1)){
	continue;
      }
	...
	// convert vertices
	gc.geo2xy(area, polydata[psize].np,
		  polydata[psize].polyverx,

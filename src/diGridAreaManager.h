...
private:
  std::map<std::string,GridArea> gridAreas;
  std::string currentId;
  std::map<std::string,GridArea> tmp_gridAreas;
  std::vector<SpatialInterpolateArea> spatialAreas;
  mapMode mapmode;
...
//	bool setGridAreas(map<std::string,Polygon> newAreas, Projection currentProj );
  ...
  std::vector<SpatialInterpolateArea> getSpatialInterpolateAreas() const {return spatialAreas;}
...
//	void setBaseProjection(Projection proj){base_proj = proj; }
  ...
  std::vector<std::string> getId(Point p);
};

    ...
    , handlingTimeMessage(false)
{
  ...
  // --------------------------------------------------------------------
  toolIdnumUpAction = new QAction(QIcon(QPixmap(idnumUp_xpm)), tr("EPS cluster/member etc up"), this);
  ...
  // --------------------------------------------------------------------
  toolIdnumDownAction = new QAction(QIcon(QPixmap(idnumDown_xpm)), tr("EPS cluster/member etc down"), this);
  ...
  // Autoupdate ===============================
  // --------------------------------------------------------------------
  autoUpdateAction = new QAction(QIcon(QPixmap(autoupdate_yellow_xpm)), tr("Automatic updates"), this);
  ...
  connect(infomenu, SIGNAL(triggered(QAction*)), SLOT(info_activated(QAction*)));
  ...
  connect(statusbuttons, SIGNAL(toggleElement(PlotElement)), SLOT(toggleElement(PlotElement)));
  connect(statusbuttons, SIGNAL(releaseFocus()), SLOT(setFocus()));
  ...
  connect(em, SIGNAL(emitFieldEditUpdate(std::string)), fm, SLOT(fieldEditUpdate(std::string)));

  // resize main window according to the active map area when using
  // an editing tool
  connect(em, SIGNAL(emitResize(int, int)), this, SLOT(winResize(int, int)));

  // HELP
  connect(stm, SIGNAL(showsource(const std::string, const std::string)), help, SLOT(showsource(const std::string, const std::string)));
  connect(mm, SIGNAL(showsource(const std::string, const std::string)), help, SLOT(showsource(const std::string, const std::string)));
  connect(em, SIGNAL(showsource(const std::string, const std::string)), help, SLOT(showsource(const std::string, const std::string)));
  connect(qm, SIGNAL(showsource(const std::string, const std::string)), help, SLOT(showsource(const std::string, const std::string)));
  connect(trajm, SIGNAL(showsource(const std::string, const std::string)), help, SLOT(showsource(const std::string, const std::string)));
...
// create a new main window
  ...
  connect(qApp, SIGNAL(aboutToQuit()), SLOT(writeLogFile()));
...
void DianaMainWindow::initCoserverClient()
{
  ...
  connect(pluginB, SIGNAL(receivedMessage(int, const miQMessage&)), SLOT(processLetter(int, const miQMessage&)));
  connect(pluginB, SIGNAL(disconnected()), SLOT(connectionClosed()));
  connect(pluginB, SIGNAL(renamed(const QString&)), SLOT(setInstanceName(const QString&)));
}
...
void DianaMainWindow::recallPlot(const PlotCommand_cpv& vstr, bool replace)
{
  ...
  if (replace || mapcom.size())
    mm->putOKString(mapcom);
  if (replace || fldcom.size())
    fm->putOKString(fldcom);
  if (replace || satcom.size())
    sm->putOKString(satcom);
  if (replace || obscom.size())
    om->putOKString(obscom);
  if (replace || statcom.size())
    stm->putOKString(statcom);
  if (replace || objcom.size())
    objm->putOKString(objcom);
  if (replace || labelcom.size())
    annom->putOKString(labelcom);
...
void DianaMainWindow::toggleDialogs()
{
  ...
  if (b) {
    if ((visi[0] = qm->isVisible()))
      quickMenu();
    if ((visi[1] = mm->isVisible()))
      mapMenu();
    ...
    if ((visi[7] = trajm->isVisible()))
      trajMenu();
    if ((visi[8] = measurementsm->isVisible()))
      measurementsMenu();
...
void DianaMainWindow::vcrossEditManagerEnableSignals()
{
  if (not vcrossEditManagerConnected) {
    ...
    connect(EditItemManager::instance(), SIGNAL(itemChanged(const QVariantMap&)), vcInterface.get(), SLOT(editManagerChanged(const QVariantMap&)),
            Qt::UniqueConnection);
    connect(EditItemManager::instance(), SIGNAL(itemRemoved(int)), vcInterface.get(), SLOT(editManagerRemoved(int)), Qt::UniqueConnection);
    connect(EditItemManager::instance(), SIGNAL(editing(bool)), vcInterface.get(), SLOT(editManagerEditing(bool)), Qt::UniqueConnection);
  }
}
...
void DianaMainWindow::processLetter(int fromId, const miQMessage& qletter)
{
  ...
  if (command == "menuok") {
    ...
    contr->makeStationPlot(commondesc, common, description, fromId, letter.data);
    ...
    if (desc.size() == 2) { // obsolete syntax
      contr->stationCommand("changeImageandText", letter.data, desc[0], fromId, desc[1]);
    } else { // new syntax
      // commondesc: name of dataset
      // description: stationname:image:image2:text
      //             :alignment:rotation (0-7):alpha

      contr->stationCommand("changeImageandText", letter.data, letter.common, fromId, letter.description);
    }
  }
    ...
    contr->stationCommand("showPositionText", letter.data, letter.common, fromId, letter.description);
  }
...
{
  QString str = tr("Diana - a 2D presentation system for meteorological data, including fields, observations,\nsatellite- and radarimages, vertical profiles "
...
void DianaMainWindow::setPlotTime(const miutil::miTime& t)
{
  ...
  objm->commentUpdate();
  if (vpWindow)
    vpWindow->mainWindowTimeChanged(t);
  if (spWindow)
    spWindow->mainWindowTimeChanged(t);
  if (vcInterface.get())
...
void DianaMainWindow::parseSetup()
{
  ...
  if (setupDialog.exec()) {
    ...
    if (!sp.parse(filename)) {
      ...
      QMessageBox::warning(this, tr("Error"), tr("An error occured while re-reading the setup file '%1'.").arg(QString::fromStdString(filename)));
    }
    if (DiCanvas* c = w->Glw()->canvas())
      c->parseFontSetup();
    contr->parseSetup();
...
{
  vstr.push_back(name + ".size " + saveXY(d->width(), d->height()) + " " + (d->showsMore() ? "YES" : "NO"));
}
...
void DianaMainWindow::readLog(const vector<string>& vstr, const string& thisVersion, string& logVersion)
{
  ...
  for (; ivstr < nvstr; ivstr++) {
    if (vstr[ivstr].substr(0, 4) == "====")
      break;
    tokens = miutil::split(vstr[ivstr], 0, " ");
    if (tokens[0] == "DocState")
      restoreDocState(vstr[ivstr]);
    ...
    if (tokens.size() == 3 || tokens.size() == 4) {
      ...
      if (x > 20 && y > 20 && x <= displayWidth && y <= displayHeight) {
        if (tokens[0] == "MainWindow.size")
          this->resize(x, y);
      }
      if (x >= 0 && y >= 0 && x < displayWidth - 20 && y < displayHeight - 20) {
        if (tokens[0] == "MainWindow.pos")
          this->move(x, y);
        else if (tokens[0] == "QuickMenu.pos")
          restoreDialogPos(qm, x, y);
        else if (tokens[0] == "FieldDialog.pos")
          restoreDialogPos(fm, x, y);
        else if (tokens[0] == "FieldDialog.size")
          restoreDialogSize(fm, showmore, x, y);
        else if (tokens[0] == "ObsDialog.pos")
          restoreDialogPos(om, x, y);
        else if (tokens[0] == "SatDialog.pos")
          restoreDialogPos(sm, x, y);
        else if (tokens[0] == "StationDialog.pos")
          restoreDialogPos(stm, x, y);
        else if (tokens[0] == "EditDialog.pos")
          restoreDialogPos(em, x, y);
        else if (tokens[0] == "ObjectDialog.pos")
          restoreDialogPos(objm, x, y);
        else if (tokens[0] == "TrajectoryDialog.pos")
          restoreDialogPos(trajm, x, y);
        else if (tokens[0] == "Textview.size")
          restoreDialogSize(textview, showmore, x, y);
        else if (tokens[0] == "Textview.pos")
          restoreDialogPos(textview, x, y);
        else {
          map<QAction*, DataDialog*>::iterator it;
          for (it = dialogs.begin(); it != dialogs.end(); ++it) {

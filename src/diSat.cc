...
// Default constructor
Sat::Sat()
  : approved(false),image(0), nx(0), ny(0),
    channelschanged(true),mosaicchanged(true),autoFile(true),
    rgboperchanged(true), alphaoperchanged(true),
    classtable(defaultClasstable), alphacut(defaultAlphacut),
    alpha(defaultAlpha), cut(defaultCut),
...
void Sat::memberCopy(const Sat& rhs){
  ...
  for (int i=0; i<3; i++) rgbindex[i]= rhs.rgbindex[i];
...
void Sat::values(int x, int y, vector<SatValues>& satval)
{
  ...
  if(x>=0 && x<nx && y>=0 && y<ny && approved){ // inside image/legal image
    ...
    for(;p!=q && rawimage[p->first]!=NULL;p++){
      SatValues sv;
      sv.value = -999.99;     
      int pvalue = rawimage[p->first][index];
      //return if colour is hidden
      for( int i=0; i<hideColor.size(); i++){
	if( hideColor[i] ==pvalue) return;
      }
      if (pvalue!=0){
...
void Sat::setCalibration(){
  ...
  if (palette){
    ...
    ct.channel = name;
    ct.val.push_back(" ");
    ct.val.insert(ct.val.end(),
		  paletteInfo.clname.begin(),

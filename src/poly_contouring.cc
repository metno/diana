...
#define LV(x) #x << "='" << x << "' "
#define DUMP_L(t,x)  P("[" t << ':' << x->get_level() << "@" << x->m_line_id << " f="); if(x->front()) P(x->front()->m_end_id); else P('%'); P(" b="); if (x->back()) P(x->back()->m_end_id); else P('%'); P(']')
#define DUMP_LE(t,x) P("{" t "@"); if (x) { P(x->m_end_id); DUMP_L("", x->get_line()); } else P("%"); P('}')
#define DUMP_TR(t,x) P("<" t "@" << x->m_triplet_id << '('); DUMP_LE("o", (x)->outer()); P(' '); \
    DUMP_LE("c", (x)->ctour()); P(' '); DUMP_LE("i", (x)->inner()); P('>')
...
// line_end objects for outer and inner line (polygons), and ctour line (polylines)
class line_triplet
{
public:
    line_triplet(line_end_x o, line_end_x c, line_end_x i)
        : m_outer(o), m_ctour(c), m_inner(i), m_peer(0), m_generation(0)
    ...
    void add(const point_t& point)
        { m_outer->add(point); m_ctour->add(point); m_inner->add(point); }
        
    level_t level_outer() const
        { return m_outer->get_level(); }
        
    level_t level_ctour() const
        { return m_ctour->get_level(); }
        
    level_t level_inner() const
        { return m_inner->get_level(); }
        
    line_end_x outer() const
        { return m_outer; }
        
    line_end_x ctour() const
        { return m_ctour; }
        
    line_end_x inner() const
        { return m_inner; }
        
    void set_outer(line_end_x b)
        { m_outer = b; }
        
    void set_ctour(line_end_x m)
        { m_ctour = m; }
        
    void set_inner(line_end_x t)
        { m_inner = t; }
        
    size_t get_generation() const
        { return m_generation; }
        
    void set_generation(size_t g)
        { m_generation = g; }
        
    void flip()
        { std::swap(m_inner, m_outer); /*if (m_peer) m_peer->flip();*/ }
...
private:
    line_end_x m_outer, m_ctour, m_inner;
...
private:
    ...
    line_triplet_x open_left(const point_t& mid_point, line_end_x b, level_t level_m, level_t level_top);
    ...
    line_triplet_x new_line_triplet(line_end_x o, line_end_x c, line_end_x i)
        { return pool_line_triplet.construct(o, c, i); }
...
#ifdef CONTOURING_ENABLE_DEBUG
size_t runner::line_end::s_end_id = 0;
size_t runner::line::s_line_id = 0;
size_t runner::line_triplet::s_triplet_id = 0;
#endif // CONTOURING_ENABLE_DEBUG
...
line_triplet_x runner::open_left(const point_t& p, line_end_x o, level_t lc, level_t li)
{
    POCO_DEBUG_SCOPE();
    POCO_DEBUG(DUMP_LE("o", o); P(LV(lc) << LV(li)));
#ifdef CONTOURING_ENABLE_DEBUG
    if (lc != o->get_level() and lc != li)
        FAIL("BMT BAD LEVEL COMBI" << LV(o->get_level()) << LV(lc) << LV(li));
#endif
    ...
    line_end_x i = new_line_end(p, li);

    return new_line_triplet(o, c, i);
}
...
triplet_pair runner::open_single(const point_t& p, level_t lo, level_t lc, level_t li)
{
    ...
    line_triplet_x bright = new_line_triplet(o2, c2, i2);
    btop->peer_up(bright);
    m_triplet_generation += 1;
    btop->set_generation(m_triplet_generation);
    bright->set_generation(m_triplet_generation);
...
{
    POCO_DEBUG_SCOPE();
    // emits ctour of bmt and joins/closes outer of bmt.inner to le_t
    ...
    line_end_x le_i = bmt->inner();
    line_end_x le_o = bmt->outer();
    if (le_t->get_level() != bmt->level_inner())
        std::swap(le_i, le_o);
...
void runner::close_inside(line_triplet_x bleft, line_triplet_x bbottom)
{
    ...
    bool is_island = bbottom->ctour()->same_line(*bleft->ctour());
    line_join(bbottom->ctour(), bleft->ctour(), false);

    line_triplet_x p_left = bleft->peer(), p_bottom = bbottom->peer();
    const size_t g_bottom = bbottom->get_generation(), g_left = bleft->get_generation();
    POCO_DEBUG(P(LV(g_bottom) << LV(g_left)));

    const bool flip_b = (bbottom->level_inner() == bleft->level_outer());
    if (flip_b)
        bbottom->flip();

    if (g_bottom < g_left) {
        bleft->set_generation(g_bottom);
        if (p_left) {
            if (flip_b)
                p_left->flip();
            p_left->set_generation(g_bottom);
        }
    } else if (g_left < g_bottom) {
        bbottom->set_generation(g_left);
        if (p_bottom) {
            if (flip_b)
                p_bottom->flip();
            p_bottom->set_generation(g_left);
        }
    }
    ...
    if (is_island and not bleft->outer()->same_line(*bbottom->outer()))
        is_island = false;

    if (is_island) {
        POCO_DEBUG(P(" .. connect_up "); DUMP_LE("cu",bleft->outer()); DUMP_LE("cuo",bbottom->outer()));
        m_connect_up = connect_up_t(bleft->outer(), bbottom->outer());
    } else {
        line_join(bleft->outer(), bbottom->outer(), true);
    }
    line_join(bleft->inner(), bbottom->inner(), true);
...
line_end_x runner::le_connect_up(line_end_x le)
{
    ...
    POCO_DEBUG(DUMP_LE("le",le); P(','); DUMP_LE("cu",cu); P(','); DUMP_LE("cuo",cuo));
    line_end_x oe_cu = cu->other_end();
    const point_t cu_last = cu->get_point_at_end();
    const point_t le_last = le->get_point_at_end();
    const bool closed = le->same_line(*cu);
    if (line_join(le, cu, true) and closed)
        return cuo;
...
void runner::connect_up(line_triplet_x above)
{
    if (m_connect_up.first) {
        ...
        // "outer" line; this must be the "outer" line for an island
        
        if (above->level_outer() != m_connect_up.first->get_level())
            above->set_inner(le_connect_up(above->inner()));
        else
            above->set_outer(le_connect_up(above->outer()));
    }
                                ...
                                level_t level_bl, level_t level_tl, level_t level_tr)
{
    ...
    if (defined_bot) {
        ...
        if (n_bottom > 0) {
            ...
            for (size_t i=0; i<n_bottom; ++i)
                undef.first->set_outer(close_right(*(--lix0), undef.first->outer()));
    ...
    if (defined_left) {
        if (n_left > 0) {
            ...
            for (size_t i=0; i<n_left; ++i, ++it_top)
                undef.second->set_outer(close_right(*it_top, undef.second->outer()));
    ...
    if (defined_right) {
        ...
        for (; not right.done(); right.next()) {
            ...
            line_triplet_x nbmt = open_left(right.point(), undef.first->outer(), right.level(), lt);
            m_triplets.insert(m_tix, nbmt);
            undef.first->set_outer(new_line_end(nbmt->inner()));
        }
    ...
    if (defined_top) {
        ...
        for (; not top.done(); top.next()) {
            ...
            line_triplet_x nbmt = open_left(top.point(), undef.second->outer(), top.level(), lt);
            undef.second->set_outer(new_line_end(nbmt->inner()));
            nbmt->flip();
    ...
    if (join_undef) {
        ...
        if ((*it_after_undef)->level_outer() == level_bl) {
            (*it_after_undef)->flip();
            undef.second->flip();
        }
    ...
    if (split_undef) {
        ...
        //    std::swap(lo, li);
#endif
        undef = open_single(p_corner_tl, lo, lc, li);
        triplet_li it1 = advanced(m_tix, 1);
        ...
        if (not (undef_bl or undef_tl)) {
          if (abs(level_bl - level_tl) > MAX_LEVELS)
            ...
            for (; not border.done(); border.next()) {
                ...
                const level_t level_in = (level_tl > level_bl) ? (level_c+1) : level_c;
                m_triplets.push_back(open_left(border.point(), m_le_border_top, level_c, level_in));
                m_le_border_top = new_line_end(m_triplets.back()->inner());
            }
            ...
            m_le_border_top = new_line_end(undef_tail->inner());
            m_le_border_top->add(point_tl);
            ...
            m_le_border_top = new_line_end(undef_head->inner());
            undef_head->flip();
        }
...
void runner::prepare_column_bottom(size_t ix)
{
    ...
    if (not (undef_bl or undef_br)) {
        ...
        for (; not bottom.done(); bottom.next()) {
            ...
            level_t level_in = level_c + bottom.increasing();
            line_triplet_x t = open_left(bottom.point(), m_le_border_low, level_c, level_in); // FIXME this actually closes the polygon le_border_low
            m_le_border_low = new_line_end(t->inner());
        ...
        line_triplet_x undef_head = open_left(p, m_le_border_low, m_level_br, m_level_br); // FIXME can this close le_border_low?
        m_le_border_low = new_line_end(undef_head->inner());
        m_le_border_low->add(point_br);
        m_triplets.push_front(undef_head);
        m_triplets.push_front(0); // tail
    } else if (undef_bl and (not undef_br)) {
        // bot border: end undef
        line_triplet_x undef_tail = open_left(point_br, m_le_border_low, level_bl, m_level_br);
        m_le_border_low = new_line_end(undef_tail->inner());
        undef_tail->flip();
    ...
    if (not (undef_bl or undef_br)) {
        ...
        if (n_exit_top) {
            ...
            for (size_t i=0; i<n_exit_top; ++i)
                m_le_border_top = close_right(*(--it_exit), m_le_border_top);
        ...
        POCO_DEBUG(P("HELO finish_column_top"));
        // top border: new undef from below
        line_triplet_x& undef_head = *advanced(m_tix, 1);
        // join undef_head.outer with m_le_border_top => undef_head.outer
        // add point_br to undef.second.top
        undef_head->inner()->add(point_br);
        m_le_border_top = close_right(undef_head, m_le_border_top);
        undef_head = 0;
    } else if (undef_bl and (not undef_br)) {
        ...
        if (not undef_head)
            m_le_border_top = close_right(undef_tail, m_le_border_top);
        else if (undef_head->inner())
            m_le_border_top = close_right(undef_tail, undef_head->inner());
        else
            m_le_border_top = 0;
        erase(it_drop, m_tix);
    }

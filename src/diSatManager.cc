...
//                -if similar plot alrede exists, just make a copy of the
...
//  PURPOSE:s   Read data from file, and init. SatPlot
  ...
  SatFileInfo & fInfo = 
    Prod[satdata->satellite][satdata->filetype].file[index];
  ...
  if (readfresh){ // nothing to reuse..
    ...
    if( !parseChannels(fInfo) ) return false; 
    satdata->cleanup();
    if (!readSatFile()) return false;
    satdata->setArea();
...
//  PURPOSE:   uses palette to put data from image into satdata.image
  ...
  // image(RGBA)
  if (satdata->image) delete[] satdata->image;
  ...
  for( int i=0; i<=ncolours; i++)
    fInfo.col.push_back(Colour
			(colmap[0][i],colmap[1][i],colmap[2][i]));
...
//   * PURPOSE:   put data from 3 images into satdata.image
  ...
  if (dorgb) {
	...
	if(color[k]!=NULL && (k==0 || color[k]!=color[k-1]) ){
	  ...
	  if(!colourStretchInfo.channels.exists() || satdata->commonColourStretch){
...
void 
SatManager::cutImage(unsigned char *image,float cut, int &index1, int &index2){
...
int SatManager::getFileName(miString &name){
  ...
  vector<SatFileInfo> &ft = 
    Prod[satdata->satellite][satdata->filetype].file;
...
int SatManager::getFileName(const miTime &time){
  ...
  if(subp.file.size() == 0 || 
     subp.updated ==false || 
     (!subp.archiveFiles && updiff > updateFreq)) {
    listFiles(subp);
    subp.updated = true;
  }
  else
...
//  * PURPOSE:   add files to existing image
  ...
  for (int i=0;i<n && !filledmosaic;i++){ //loop over mosaic files
    if (mosaicfiles[i].time < firstFileTime) firstFileTime=mosaicfiles[i].time;
    if (mosaicfiles[i].time > lastFileTime) lastFileTime=mosaicfiles[i].time;
    ...
    for (int i=0; i<Sat::maxch; i++){
      if (rawimage[i]!=0) delete[] rawimage[i];
      rawimage[i]= 0;
    }
...
#ifdef DEBUGPRINT
  cerr<<"SatManager----> readHeader "<<file.name<<endl;
#endif
...
void SatManager::listFiles(subProdInfo &subp){
   ...
   for(int j=0; j<subp.pattern.size();j++) {
     ...
     if(subp.archive[j] && !useArchive) continue;     
...
//    cerr <<"listFiles - no. of files:"<<subp.file.size()<<endl;
  ...
  int i=0;
  while(i<n && i<3 && 
	!MItiff::readMItiffPalette(subp.file[i].name.c_str(),subp.colours))
    i++;
...
//  * PURPOSE:   Info to fro setup
  ...
  for(int i=0; i<sect_sat.size(); i++) {
	...
	channels.push_back(vstr[0]);
      ...
      while(iprod<n && Dialog.image[iprod].name != value) iprod++;
      if(iprod==n){
	...
	continue;
      ...
      while(j<n && Dialog.image[iprod].file[j].name != value) j++;
      if(j==n){

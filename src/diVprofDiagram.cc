...
VprofDiagram::VprofDiagram(VprofOptions *vpop)
  : VprofTables(), vpopt(vpop), drawlist(0), diagramInList(false),
    numtemp(0), numprog(0)
{
#ifdef DEBUGPRINT
...
void VprofDiagram::plot()
{
  ...
  if (vpopt->changed) newdiagram= true;

  bool redraw= newdiagram;
  if (vpopt->changed) redraw= true;
  if (!diagramInList) redraw= true;
  ...
  if (redraw) {
    if (diagramInList && glIsList(drawlist)) glDeleteLists(drawlist,1);
    if (newdiagram) prepare();
    diagramInList= false;
  }
...
//const int k700 = 700/idptab;
  ...
  if (pmaxDiagram-pminDiagram<idptab*2) pminDiagram= pmaxDiagram-idptab*2;
  if (pminDiagram<0)                    pminDiagram= 0;
  if (pmaxDiagram-pminDiagram<idptab*2) pmaxDiagram= pminDiagram+idptab*2;
  if (pmaxDiagram>idptab*(mptab-1))     pmaxDiagram= idptab*(mptab-1);
  ...
  if (vpopt->diagramtype!=last_diagramtype || vpopt->tangle!=last_tangle) {
    ...
    if (vpopt->diagramtype==0) {
      ...
      // 490 - 1300 hPa:
      for (k=k500-2; k<mptab; k++) yptabd[k]= pplog[k];
      // 0 - 495 hPa:
	...
	yptabd[k]= yptabd[k500] + (k500-k)*d5hpa;
      ...
      // exner-funksjonen (pi)
      for (k=0; k<mptab; k++) yptabd[k]= pitab[k];
    } else if (vpopt->diagramtype==2) {
      // p
      for (k=0; k<mptab; k++) yptabd[k]= pptab[k];
    } else if (vpopt->diagramtype==3) {
      // ln(p)
      for (k=0; k<mptab; k++) yptabd[k]= pplog[k];
    }
  ...
  if (vpopt->rsvaxis!=last_rsvaxis ||
      pmin!=last_pmin || pmax!=last_pmax) {
...
//}
  ...
  float ymindf= ymind;
  float ymaxdf= ymaxd;
  if (vpopt->plabelp && vpopt->pplines) xmindf-=chxlab*5.5;
  ...
  if (vpopt->plabelq) ymindf-=(chylab*0.8*1.5);
  ...
  for (n=3; n<mxysize-1; n++) {
    if (ymin>xysize[n][2]) ymin= xysize[n][2];
    if (ymax<xysize[n][3]) ymax= xysize[n][3];
  }
  if (numtemp+numprog>1 && (vpopt->pwind || vpopt->pslwind)) {
    if (ymin>ymind-chylab*2.) ymin= ymind-chylab*2.;
  }
  if (vpopt->plabelp || (vpopt->pflevels && vpopt->plabelflevels)) {
    if (ymin>ymind-chylab*2.) ymin= ymind-chylab*2.;
  }
  if (vpopt->pslwind) {
    if (ymin>ymind-chylab*0.85) ymin= ymind-chylab*0.85;
    if (ymax<ymaxd+chylab*0.85) ymax= ymaxd+chylab*0.85;
  }
  for (n=3; n<mxysize-1; n++) {
    xysize[n][2]= ymin;
    xysize[n][3]= ymax;
  ...
  if (vpopt->ptext) {
    float ytext= ymin;
    n= numtemp+numprog;
    if (n<vpopt->linetext) n= vpopt->linetext;
    if (n<1) n=1;
...
//float xmaxdf= xysize[2][1];
  ...
  float x1,x2,y1,y2,dx,dy,x,y,p,t,chx,chy,numrot,dx1,dx2,dxmin,dymin;
...
//         fp->drawStr("FL",x1-cw,ymind-chylab*1.25,0.0);
	...
	// pressure levels
	int ipstep,ip;
        if      (pmax-pmin>399.) ipstep=100;
        else if (pmax-pmin>199.) ipstep=50;
        else if (pmax-pmin> 99.) ipstep=25;
        else                     ipstep=10;
        for (k=0; k<kk; k++) {
	  if (ylev[k]>y1 && ylev[k]<y2) {
...
//         fp->drawStr("hPa",x1-cw,ymind-chylab*1.25,0.0);
  ...
  if (vpopt->ptlines) {
    ...
    if (it1<itmin) it1+=tStep;
    if (it2>itmax) it2-=tStep;
    ...
    if (vpopt->plabelt) {
      ...
      if (itstep<vpopt->tStep) itstep= vpopt->tStep;
      ...
      if (itmin>0) it1+=itstep;
      if (itlim<0) it2-=itstep;
      if (it1>-100 && it2<+100) {
        chx= chxlab;
        chy= chylab;
	numwid= 3;
...
//       setFontsize(chy);
      ...
      for (it=it1; it<=it2; it+=itstep) {
        ...
        if (x>xnext) {
	  ostringstream ostr;
	  ostr << setw(numwid) << setfill(' ')
	       << setiosflags(ios::showpos) << it;
	  miString str= ostr.str();
//           fp->drawStr(str.c_str(),x,y,numrot);
	...
	ylast= ymindf+chy*costan-0.5*chy/sintan;
        ...
        dx= 0.5*chxlab;
        dy= 0.5*chy;
	if (xlast<xmaxd) ynext= ymaxdf;
	else             ynext= ymaxd-chy*0.5;
...
//           fp->drawStr(str.c_str(),x,y,numrot);
  ...
  for (k=kpmin; k<=kpmax; k++) yline[k]= yptab[k];
  ...
  if (vpopt->pdryadiabat) {
    ...
    if (itt1>0) it1+=itstep;
    if (itt2<0) it2-=itstep;
    if (vpopt->diagramtype!=1) {
      for (it=it1; it<=it2; it+=itstep) {
        th= t0+it;
	k1= kpmin;
	if (k1<k25  && it%(itstep*8)!=0) k1=k25;
	if (k1<k50  && it%(itstep*4)!=0) k1=k50;
	if (k1<k100 && it%(itstep*2)!=0) k1=k100;
        for (k=k1; k<=kpmax; k++) {
  ...
  if (vpopt->pwetadiabat) {
    ...
    if (kmin<kpmin) kmin= kpmin;
    if (kmin<kpmax) {
      float tempmin= vpopt->wetadiabatTmin;
      int mintemp= (vpopt->wetadiabatTmin>itmin1000) ? vpopt->wetadiabatTmin : itmin1000;
      it1= (mintemp/itstep)*itstep;
      it2= (itmax/itstep)*itstep;
      if (mintemp>0) it1+=itstep;
      if (itmax<0)   it2-=itstep;
  ...
  if (vpopt->pmixingratio) {
    ...
    if (set<0 || set>=vpopt->qtable.size()) set= 1;
    ...
    if (kmin<kpmax) {
      ...
      for (int iq=0; iq<nq; iq++) {
        ...
        for (k=kmin; k<=kpmax; k++) {
          ...
          while (ewt[n]<esat && n<mewtab-1) n++;
          x= (esat-ewt[n-1])/(ewt[n]-ewt[n-1]);
          t= -100.+(n-1.+x)*5.;
          xline[k]= xztab[k]+dx1deg*t;
          if (t<tempmin) kstop= k;
        }
        if (kstop==0) {
          kstop= kmin;
	  ytmp= yline[kstop];
	  ...
	  ytmp= yline[kstop];
          ...
          x= (tempmin-t1)/(t2-t1);
          xline[kstop]= xline[kstop+1]
                       +(xline[kstop]-xline[kstop+1])*x;
          yline[kstop]= yline[kstop+1]
                       +(yline[kstop]-yline[kstop+1])*x;
        } else {
	  ytmp= yline[kstop];
	}
        if (kpmax-kstop+1>1)
...
//       fp->drawStr("FL",x1,ymind-chy*1.25,0.);
  ...
  if (vpopt->pvwind && numprog>0) {
    ...
    if (dx>chxlab*0.8) dx= chxlab*0.8;
    dy= (xysize[6][3]-ymaxd)*0.5;
    if (dy>chylab*0.9) dy= chylab*0.9;
...
//     fp->drawStr(str.c_str(),xysize[6][0]+(dx-cw)*0.5,ymind-chy*1.2,0.);
    fpInitStr(str.c_str(),xysize[6][0]+dx*0.5,ymind-chy*1.2,0.,
	      chy,c2,"x-w*0.5");
//     fp->getStringSize("hPa/s", cw, ch);
//     fp->drawStr("hPa/s",xysize[6][0]+(dx-cw)*0.5,ymind-chy*2.4,0.);
    fpInitStr("hPa/s",xysize[6][0]+dx*0.5,ymind-chy*2.4,0.,chy,c2,"x-w*0.5");
    glColor3ubv(c.RGB());
    sepline= true;
    UpdateOutput();
  }
...
//     fp->drawStr(str3.c_str(),xysize[8][1]-cw3-cw2*0.3,ymind-1.25*chy,0.0);
    fpInitStr(str1.c_str(),xysize[8][0],ymind-1.25*chy,0.0,ch,c2,"ductingmin");
    fpInitStr(str3.c_str(),xysize[8][1],ymind-1.25*chy,0.0,ch,c2,"ductingmax");
    glColor3ubv(c.RGB());
    sepline= true;
    UpdateOutput();
  }
...
void VprofDiagram::fpInitStr(const miString& str,
...
void VprofDiagram::fpDrawStr(bool first)
{
  ...
  for(int i=0; i<n;i++){
    ...
    if(first && fpStr[i].format.exists()){
      ...
      if(fpStr[i].format=="x-w") fpStr[i].x -= w;
      if(fpStr[i].format=="x-w*0.5") fpStr[i].x -= w*0.5;
    ...
    fp->drawStr(fpStr[i].str.c_str(),
		fpStr[i].x,fpStr[i].y,fpStr[i].z);
    //reset font

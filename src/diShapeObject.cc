...
ShapeObject::ShapeObject(const ShapeObject &rhs) : ObjectPlot(rhs){
}


bool ShapeObject::read(miString filename){
  cerr << "ShapeObject::read" << filename << endl;
  ...
  for( i = 0; i < nEntities; i++ )
    ...
    int idbf= readDBFfile(filename,
			dbfIntName,     dbfIntDesc,
			dbfDoubleName,  dbfDoubleDesc,
			dbfStringName,  dbfStringDesc);
...
/* -------------------------------------------------------------------- */
    ...
    for (n=0; n<dbfDoubleName.size(); n++) {
	  ...
	  if (iRecord < 10) cerr   << "DBFReadDoubleAttribute( hDBF, iRecord, i )" << DBFReadDoubleAttribute( hDBF, iRecord, i ) << endl;
	}
      }
    }
    ...
    for (n=0; n<dbfStringName.size(); n++) {
	  ...
	  dbfStringDesc[n].push_back("-");
          ...
          dbfStringDesc[n].push_back( miString( DBFReadStringAttribute( hDBF, iRecord, i ) ) );
	  if (iRecord < 10) cerr   << "DBFReadStringAttribute( hDBF, iRecord, i )" << DBFReadStringAttribute( hDBF, iRecord, i ) << endl;
	}
      }
    }
    ...
    int m= 0;
    while (m<dbfDoubleName.size() && dbfDoubleName[m]!="LENGTH") m++;
    ...
    if(m<dbfDoubleName.size()) {
      ...
      if (n>0) {
        ...
        for (i=0; i<n; i++) {
          if (minlength>dbfDoubleDesc[m][i]) minlength= dbfDoubleDesc[m][i];
          if (maxlength<dbfDoubleDesc[m][i]) maxlength= dbfDoubleDesc[m][i];
          avglength+=dbfDoubleDesc[m][i];
	  if (dbfDoubleDesc[m][i]<1.0) n1++;
	  if (dbfDoubleDesc[m][i]<10.0) n10++;
	  if (dbfDoubleDesc[m][i]<100.0) n100++;
	  if (dbfDoubleDesc[m][i]<1000.0) n1000++;
	}
        avglength/=double(n);
      }
    }
    ...
    for (n=0; n<dbfIntName.size(); n++)
      cerr<<"Int    description, size,name:  "<<dbfIntDesc[n].size()<<"  "<<dbfIntName[n]<<endl;
    for (n=0; n<dbfDoubleName.size(); n++)
      cerr<<"Double description, size,name:  "<<dbfDoubleDesc[n].size()<<"  "<<dbfDoubleName[n]<<endl;
    for (n=0; n<dbfStringName.size(); n++)
      cerr<<"String description, size,name:  "<<dbfStringDesc[n].size()<<"  "<<dbfStringName[n]<<endl;

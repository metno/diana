...
bool AnnotationPlot::prepare(const miString& pin){
  ...
  for (i=1; i<n; i++){
    ...
    if (labeltype.upcase()=="DATA"){
      ...
      if (a.str[0]=='"') a.str= a.str.substr(1,a.str.length()-2);
...
bool AnnotationPlot::setData(const vector<Annotation>& a,
  ...
  if (atype!=anno_text) annotations= a;
  if (useAnaTime) fieldAnaTime= fieldAnalysisTime;
  splitAnnotations();
  putElements();
...
bool AnnotationPlot::putElements(){
  ...
  for (int i=0; i<n; i++){
    ...
    annotations[i].wid=0;
    annotations[i].hei=0;
    if(annotations[i].str.empty()) continue;
	...
	for (int k=0;k<mtokens;k++){
	  ...
	  if (subtokens[0]=="horalign"){
	    if (subtokens[1]=="center") annotations[i].hAlign=align_center;
	    if (subtokens[1]=="right") annotations[i].hAlign=align_right;
	    if (subtokens[1]=="left") annotations[i].hAlign=align_left;
	  }
	}
	...
	vector <miString> stokens=elementstrings[l].split('=');
	if (stokens.size()!=2) continue;
	if (stokens[1]=="fill") annotations[i].polystyle= poly_fill;
	else if (stokens[1]=="border") annotations[i].polystyle= poly_border;
	else if (stokens[1]=="both") annotations[i].polystyle= poly_both;
	else if (stokens[1]=="none") annotations[i].polystyle= poly_none;
	...
	if(annotations[i].oldFormat){
	  ...
	  if (elementstrings[l].compare(0,3,"box")==0)subel++;
	}
      }
    }
...
void AnnotationPlot::addElement2Vector(vector<element>& v_e, 
				       const element& e,int index)
{
...
bool AnnotationPlot::decodeElement(miString elementstring, element& e)
{
  ...
  for (int k=0;k<mtokens;k++){
	...
	else if (subtokens[1]=="right")  e.eHalign=align_right;
	else if (subtokens[1]=="left") e.eHalign=align_left;
      }	     
      else if (subtokens[0]=="pstyle"){
	if (subtokens[1]=="fill")        e.polystyle= poly_fill;
	else if (subtokens[1]=="border") e.polystyle= poly_border;
	else if (subtokens[1]=="both")   e.polystyle= poly_both;
	else if (subtokens[1]=="none")   e.polystyle= poly_none;
      }
...
bool AnnotationPlot::plot(){
...
#endif
  if (!enabled || !annotations.size() || nothingToDo) return false;
  ...
  if (poptions.polystyle!=poly_border && poptions.polystyle!=poly_none){
    if (rgbmode) glColor4ubv(fc.RGBA());
    else glIndexi(fc.Index());
  ...
  if (poptions.polystyle!=poly_fill && poptions.polystyle!=poly_none){
    if (rgbmode) glColor4ubv(poptions.bordercolour.RGBA());
    else glIndexi(poptions.bordercolour.Index());    
  ...
  for (int i=0; i<n; i++){
    if (annotations[i].spaceLine) {
      ...
      y-=anno.hei;
      if (anno.hAlign==align_center) x+=0.5*(maxwid-anno.wid);
      if (anno.hAlign==align_right) x+=(maxwid-anno.wid);
      //draw one annotation - one line
      Colour c= anno.col;
      if (c==backgroundColour) c= backContrastColour;
      currentColour = c;
      if (rgbmode) glColor4ubv(c.RGBA());
      else glIndexi(c.Index());
...
bool AnnotationPlot::plotElements(vector<element>& annoEl,
				  float& x, float& y,
				  float annoHeight,
				  bool horizontal)
{
  ...
  for (int j=0;j<nel;j++){
    ...
    if (rgbmode) glColor4ubv(currentColour.RGBA());
    else glIndexi(currentColour.Index());
  ...
  UpdateOutput();
	...
	y+=annoEl[j].height-annoEl[j].subelement[0].height;
      plotElements( annoEl[j].subelement,x,y,
		    annoEl[j].height,annoEl[j].horizontal );	
      if(horizontal)
	y=tmpy;
      else
	x=tmpx;
	...
	if (annoEl[j].isInside){
      ...
      fp->set(poptions.fontname,annoEl[j].eFace,annoEl[j].eSize*fontsizeToPlot);
	...
	glEnd();
      ...
      fp->set(annoEl[j].eFont,annoEl[j].eFace,annoEl[j].eSize*fontsizeToPlot);
      float tmpwid;
      ...
      fp->set(poptions.fontname,poptions.fontface,annoEl[j].eSize*fontsizeToPlot);
      miString astring=" ";
      fp->drawStr(astring.cStr(),x,y,0.0);
      fp->getStringSize(astring.cStr(), wid, hei);
      wid*=fontsizeScale;
      ...
      ig.plotImage(annoEl[j].eImage,x+wid/2,y+hei/2,true,scale,annoEl[j].eAlpha);
...
float AnnotationPlot::plotArrow(float x, 
				float y, 
				float l, 
				bool feather)
{
...
void AnnotationPlot::getAnnoSize(vector<element> &annoEl, 
				 float& wid, float& hei, 
				 bool horizontal){
#ifdef DEBUGPRINT
  ...
  for (int j=0;j<nel;j++){
	...
	fontsizeScale = 1.0;
      ...
      fp->set(annoEl[j].eFont,poptions.fontface,annoEl[j].eSize*fontsizeToPlot);
...
void AnnotationPlot::getXYBox(){
  ...
  if (wid>maxwid) maxwid= wid;
    ...
    totalhei+=hei;
    if (annotations[i].spaceLine) totalhei-=(hei+spacing)*0.5;
  }
  maxwid += 2*border;
  ...
  if (poptions.v_align==align_bottom){
    ...
    bbox.y1 = (bbox.y1+bbox.y2)/2.0 -
      (totalhei + yoffset)/2.0;
  }
...
bool AnnotationPlot::markAnnotationPlot(int x, int y){
  ...
  if (xpos>bbox.x1 && xpos<bbox.x2 && ypos>bbox.y1 && ypos<bbox.y2){
	...
	if (annoEl.eType==input){
	  if (xpos>annoEl.x1 && xpos<annoEl.x2 &&
	      ypos>annoEl.y1&& ypos<annoEl.y2)
...
vector<miString> AnnotationPlot::split(const miString eString

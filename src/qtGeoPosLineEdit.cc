							 ...
							 int& pos) const
{
  ...
  while (1){
	...
	if (hascomma || hassemicolon) return QValidator::Invalid;
	hascomma= true;

      } else if (input[i]==':'){ // we found a semicolon
	if (hascomma || numsemicolons>1) return QValidator::Invalid;
	hassemicolon= true;
	numsemicolons++;

      } else if (input[i]=='+' || input[i]=='-'){ // we found a sign!
	if (hassign || hasletter) return QValidator::Invalid;
	hassign= true;

      } else if (input[i]=='S' || input[i]=='W' ||
		 input[i]=='N' || input[i]=='E'){ // we found a letter
	// last character only
	if ((i < n-1) && !input[i+1].isSpace()) return QValidator::Invalid;
					   ...
					   bool isLat) const
{
  ...
  if (s.contains(":")){ // degrees:minutes:seconds
	...
	if (tval > 59.999999) return false;
	tval*= asign;
	if (k==1) tval /= 60.0; // minutes
	else if (k==2) tval /= 3600.0; // seconds
      }
      testval+= tval;
    }

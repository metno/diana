...
void EditManager::sendKeyboardEvent(const keyboardEvent& me,
  ...
  int keyRegMin= key_1;
  int keyRegMax= (numregs<10) ? key_0+numregs : key_9;

  if (showRegion>=0 && mapmode!=combine_mode) {
    if (me.type!=keypress || me.key<keyRegMin || me.key>keyRegMax)
      return;
  }
  ...
  if (me.type == keypress){
    ...
    else if (me.key>=keyRegMin && me.key<=keyRegMax){
...
void EditManager::initEditTools(){
  ...
  fronts.push_back(newEditToolInfo("Station�r front",Stationary,"grey50"));
  fronts.push_back(newEditToolInfo("Tr�g",TroughLine,"black"));
#else
  ...
  fronts.push_back(newEditToolInfo("Molngr�ns",SigweatherFront,"green4"));
  ...
  fronts.push_back(newEditToolInfo("Jetstr�m",ArrowLine,"grey50"));
  fronts.push_back(newEditToolInfo("Asklinje r�d",Line,"red","red",0,false));
  fronts.push_back(newEditToolInfo("Asklinje gr�n",Line,"green","green",0,false,"dash3"));
  fronts.push_back(newEditToolInfo("Asklinje bl�",Line,"blue","blue",0,false,"dot"));
#else
  ...
  symbols.push_back(newEditToolInfo( "Showers",109,"green4"));
  ...
  areas.push_back(newEditToolInfo("Regnomr�de",Genericarea,"white"));
  ...
  //snow showers
  sigsymbols.push_back(newEditToolInfo( "Snow_showers",1043,"green4","green4",2));
  //showers
  sigsymbols.push_back(newEditToolInfo( "Showers",1044,"green4","green4",2));
...
void EditManager::setMapmodeinfo(){
  ...
  for (int i=0;i<m;i++){
    if (EdProd.drawtools[i]==OBJECTS_ANALYSIS){
      ...
      dMode.push_back(newEditModeInfo("Omr�de",areas));

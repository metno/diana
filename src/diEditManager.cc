...
void EditManager::sendKeyboardEvent(QKeyEvent* ke, EventResult& res)
{
  ...
  if (mapmode!= fedit_mode){
    ...
    if (ke->type() == QEvent::KeyPress){
      ...
      else if (ke->key() == Qt::Key_V && ke->modifiers()==Qt::Key_Control)
        objm->editPasteObjects();
      else if (ke->key() == Qt::Key_C && ke->modifiers()==Qt::Key_Control)
      ...
      else if (ke->key()==Qt::Key_Plus && ke->modifiers()==Qt::Key_Control){
        objm->editRotateObjects(+10.0);
      }
      else if (ke->key()==Qt::Key_Minus && ke->modifiers()==Qt::Key_Control){
        objm->editRotateObjects(-10.0);
      }
      ...
      else if (ke->key()==Qt::Key_B && ke->modifiers()==Qt::Key_Shift)
        //set to default size...
...
void EditManager::initEditTools(){
  ...
  symbols.push_back(newEditToolInfo( "Sn�by",114,"green4","green4",4));
  ...
  symbols.push_back(newEditToolInfo( "L�gtryck",242,"black","black",1));
